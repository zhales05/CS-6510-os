...
public class OperatingSystem implements Logging {
    ...
    // Queue for processes blocked on semaphores
    private final LinkedList<ProcessControlBlock> blockedQueue = new LinkedList<>();
    ...
    private static Set<String> getQuantumPairs() {
        ...
        quantumPairs.add("2,4");  
        quantumPairs.add("5,10"); 
        quantumPairs.add("10,20"); 
        quantumPairs.add("15,30");
        quantumPairs.add("30,60"); 
        quantumPairs.add("50,100");

        quantumPairs.add("6,12");
        quantumPairs.add("8,16");  
        quantumPairs.add("10,30");
        quantumPairs.add("20,60"); 
        quantumPairs.add("40,100");
        quantumPairs.add("50,150");
        quantumPairs.add("60,120");
        quantumPairs.add("80,160");
        quantumPairs.add("120,180");
        quantumPairs.add("150,250");

        quantumPairs.add("5,12");  
        quantumPairs.add("10,24");  
        quantumPairs.add("15,36"); 
        quantumPairs.add("35,75");  

        quantumPairs.add("100,200");
        quantumPairs.add("125,250");
        quantumPairs.add("150,300");

        quantumPairs.add("3,15"); 
        quantumPairs.add("50,200");
        return quantumPairs;
    }

    public void blockProcess(ProcessControlBlock pcb) {
        log("Blocking process " + pcb.getPid());
        pcb.setStatus(ProcessStatus.BLOCKED, QueueId.BLOCKED_QUEUE);
    ...
    public void unblockProcess(ProcessControlBlock pcb) {
        log("Unblocking process " + pcb.getPid());
        blockedQueue.remove(pcb);
        
        // Add the process to the job queue, which will eventually put it in the ready queue
        scheduler.addToJobQueue(pcb);
    }
